...
const createResearchPaperTable = `CREATE TABLE IF NOT EXISTS research_papers (paper_id INT AUTO_INCREMENT, paper_title VARCHAR(250), 
conference VARCHAR(250), publish_date DATE, CONSTRAINT pk_research_papers PRIMARY KEY(paper_id))`;

const createPapersDetails = `CREATE TABLE IF NOT EXISTS papers_details (id INT AUTO_INCREMENT, author_no INT, paper_id INT, PRIMARY KEY(id), 
CONSTRAINT fk_auth FOREIGN KEY (author_no) REFERENCES authors(author_no), CONSTRAINT fk_paper FOREIGN KEY (paper_id) REFERENCES research_papers(paper_id))`;
...
const seedDatabase = async () => {
    ...
    try {
        ...
        const authorsPromise = authors.forEach((author) =>
        ...
        const researchPapersPromise = researchPapers.forEach((paper) =>
        ...
        const papersDetailsPromise = researchPapersDetails.forEach((paper) =>
            execQuery("INSERT INTO papers_details SET ?", paper)
        );
